    ...
    <div class="row">
        <div class="content-box-large">
            <div class="panel-heading">
                <div class="panel-title" style="font-size:20px">お知らせ一覧(管理画面)</div>
			</div>
		  	<div class="panel-body">
				...
				<fieldset>
			        ...
			        <form class="form-inline" role="form" method="GET" action="{{ url('/admin/admin-news/search') }}">
                    	...
                    	@if(!empty($newsList))
							@foreach($newsList as $news)
		                        <tr>
		                            ...
		                            <td>
			                            <a href="/admin/admin-news/modify?id={{ $news->id }}">
			                                <button type="button" class="btn btn-warning btn-xs">編集</button>
			                            </a>
										@if(!$news->delete_flag)
		                               		<a href="/admin/admin-news/delete?id={{ $news->id }}" onClick="javascript:return confirm('本当に削除しますか？')">
		                               			<button type="button" class="btn btn-danger btn-xs">削除</button>
		                               		</a>
		                               	@else
											<a href="/admin/admin-news/insert?id={{ $news->id }}" onClick="javascript:return confirm('本当に復活させますか？')">
