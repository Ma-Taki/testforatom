...
@endif
                ...
                <div class="row">
                    <div class="col-md-4">案件情報入力（<font color="#FF0000">*</font>は必須入力）</div>
                </div>
...
@foreach($master_search_categories as $search_category)
@if(!isset($search_category->parent_id))
                                <label class="checkbox">
                                    <input type="checkbox" name="search_categories[]" value="{{ $search_category->id }}" {{ HtmlUtility::isChecked(old('search_categories'), $search_category->id) }}>{{ $search_category->name }}
                                </label>
@else
                                <label class="checkbox-inline">
                                    <input type="checkbox" name="search_categories[]" value="{{ $search_category->id }}" {{ HtmlUtility::isChecked(old('search_categories'), $search_category->id) }}>{{ $search_category->name }}
